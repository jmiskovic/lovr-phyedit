...
m.options = {
  ...
  show_shapes = false,           -- draw collider shapes (mesh and terrain not supported!)
  ...
  geometry_segments = 24,       -- complexity of rendered geometry (number of segments in spheres, circles, cylinders, cones)
  -- simulation parameters,
  framerate = 60,               -- inverse value
  ...
  next_color_index = 1 -- index of last chosen palette color
}
m.options.__index = m.options

m.render_shapes = setmetatable({ -- options preset for drawing filled shapes only
  wireframe = false,
  overdraw = false,
  show_shapes = true,
...
m.render_wireframe = setmetatable({ -- options preset for overdrawing the white wireframe
  geometry_segments = 24,
  shapes_palette = {{1, 1, 1, 0.05}},
  shape_colors = {},
  next_color_index = 1,
}, m.options)
...
local tmat4 = lovr.math.newMat4()

function m.update(world)
  if m.options.framerate > 0 then
    world:update(1 / m.options.framerate)
  end
end
...
function m.drawCollider(pass, collider, options)
  for _, shape in ipairs(collider:getShapes()) do
    if not options.shape_colors[shape] then
      ...
      if userdata and type(userdata) == 'table' and userdata.color then
        options.shape_colors[shape] = userdata.color
      else
        options.shape_colors[shape] = options.shapes_palette[options.next_color_index]
        options.next_color_index = 1 + (options.next_color_index % #options.shapes_palette)
      end
    end
    pass:setColor(options.shape_colors[shape])
    local pose = tmat4:set(collider:getPose()):translate(shape:getPosition()):rotate(shape:getOrientation())
    ...
    elseif userdata and userdata.model then
      pass:setColor(1,1,1)
      pass:draw(userdata.model, pose:scale(userdata.scale or 1))
